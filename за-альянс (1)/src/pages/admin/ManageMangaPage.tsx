...
import { Manga, MangaFormData, Chapter, User, Notification } from '../../../types';
...
const ManageMangaPage: React.FC<ManageMangaPageProps> = ({ manga }) => {
    ...
    const handleAddOrUpdateChapterMeta = (chapterData: Chapter) => {
        const existing = currentMangaData.chapters.find(c => c.id === chapterData.id);
        let updatedChapters;
        if (existing) {
            // Update existing chapter
            updatedChapters = currentMangaData.chapters.map(c => c.id === chapterData.id ? chapterData : c);
    ...
    const handleDeleteChapter = () => {
        ...
        const updatedChapters = currentMangaData.chapters.filter(c => c.id !== chapterToDelete.id);
        updateChapters(manga.id, updatedChapters);
    ...
    const handleBulkUpload = ({ chapters: newChapters }: { rootName: string, chapters: Chapter[] }) => {
        const existingChapterNumbers = new Set(currentMangaData.chapters.map(c => c.chapterNumber));
        const uniqueNewChapters = newChapters.filter(c => !existingChapterNumbers.has(c.chapterNumber));
