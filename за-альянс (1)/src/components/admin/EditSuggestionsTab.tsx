...
const DiffViewer: React.FC<{ original: Manga, suggestion: MangaFormData }> = ({ original, suggestion }) => {
    ...
    return (
        <div className="space-y-3 text-sm">
            {changes.map(field => (
                <div key={field}>
                    <strong className="capitalize text-text-secondary">{field}:</strong>
                    <div className="grid grid-cols-2 gap-2 mt-1">
                        <div className="bg-red-900/30 p-2 rounded">
                            <p className="text-red-300 line-through text-xs">{String(original[field as keyof Manga])}</p>
                        </div>
                         <div className="bg-green-900/30 p-2 rounded">
                            <p className="text-green-300 text-xs">{String(suggestion[field])}</p>
                        </div>
                    </div>
                </div>
            ))}
                        ...
                        <tr key={suggestion.id} className="hover:bg-overlay transition-colors">
                            ...
                            <td className="p-4">
                                <button 
                                    ...
                                    className="text-sm bg-base hover:bg-blue-900/50 text-brand font-semibold py-1 px-3 rounded-md transition-colors"
                                >
                                    Рассмотреть
                                </button>
                            </td>
